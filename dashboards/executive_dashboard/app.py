...
def apply_sidebar_filters(data: pd.DataFrame) -> pd.DataFrame:
    ...
    # 3. Patient Category (Age Group) Filter
    if "age_group" in filtered_data.columns:
        age_groups = sorted(filtered_data["age_group"].dropna().astype(str).unique().tolist())
        selected_ages = st.sidebar.multiselect(
            "Patient Category (Age Group)",
            options=age_groups,
            default=age_groups,
        )
        if selected_ages:
            filtered_data = filtered_data[filtered_data["age_group"].astype(str).isin(selected_ages)]
        else:
            filtered_data = filtered_data.iloc[0:0]
...
if filtered_data.empty:
    st.warning("No records match the selected filters. Please change your selections.")
    st.stop()
